function plot1() {
  ...
  //Read the data
  d3.csv('/static/csv/plot1.csv', function(data) {
    ...
    svg.append('g')
      ...
      .enter()
      .append("circle")
        .attr("cx", function (d) { return x(d.activities_count); } )
...
function plot2() {
  ...
  // Parse the Data
  d3.csv("/static/csv/plot2.csv", function(data) {

  // X axis
  var x = d3.scaleBand()
    .range([ 0, width ])
    .domain(data.map(function(d) { return d.student_id; }))
    .padding(0.2);
  svg.append("g")
    .attr("transform", "translate(0," + height + ")")
    .call(d3.axisBottom(x))
  ...
  svg.selectAll("mybar")
    ...
    .enter()
    .append("rect")
      .attr("x", function(d) { return x(d.student_id); })
